...
def retrieve_page(url):
    ...
    Check in local folder if the page is already downloaded,
...
def scrap_page(url):
    """
    Scrap a website page from openmindfulness.net
    and return a json document containing all contents in a structured format
    """
...
def collect_toc_links():
    ...
    page_contents, _ = retrieve_page(url)
    ...
    for link in soup.find_all("a"):
        ...
        base_url = "https://www.openmindfulness.net/"
        if link_url == base_url:
            continue
        elif link_url in [
            f"{base_url}suivre-lentrainement-online/",
            f"{base_url}contact/",
            f"{base_url}les-seminaires-retraites-online-et-residentiel/",
        ]:
            continue
...
def scrap_website():
    # sourcery skip: for-append-to-extend, inline-immediately-returned-variable,
    # sourcery skip: list-comprehension
    """
    Scrap the website openmindfulness.net and
    return a list of json documents containing all contents in a structured format
    """
    # collect all links from the Table of Contents of the website
...
def scrap_website_and_store_contents():
    """Runs data processing scripts to scrap data from openmindfulness website and
    store them into a csv file
    (saved in data/interim/openmindfulness_contents.csv).
    """
    ...
    # add a counter column to keep track of the order of the documents
    # for each unique page_chapter
